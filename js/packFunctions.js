...
async function deletePack(event, pack_url) {
  ...
  getInstalledPacks().then((data) => {
    ...
    fs.writeFileSync(
      `${currentDirectory}/packfiles/installedPacks.json`,
...
async function downloadPack(event, url, packData) {
  return new Promise((resolve, reject) => {
    ...
    downloadFile(event, download_url, directory_path, file_name).then(() => {
      getInstalledPacks().then((installed_packs) => {
...
async function activatePack(event, url) {
  ...
  return new Promise((resolve, reject) => {
    unzipFile(zipPath, targetPath).then(() => {
      copyFile(targetPath, getDBDPathSync() + dbd_icon_path).then(() => {
...
function resetAllPacks() {
  ...
  getDirectoriesInPath(`${currentDirectory}/packfiles`).forEach((directory) => {
    if (fileExists(`${currentDirectory}/packfiles/${directory}/.active`)) {
      ...
      console.log(
        `[resetAllPacks] Pack '${directory}' is not active. Skipping...`,
      );
    }
  });
}
...
function getPackMetaData(event, pack_url) {
  return new Promise((resolve, reject) => {
    ...
    if (fileExists(`${currentDirectory}/packfiles/${pack_url}/metadata.json`)) {
      pack_data = JSON.parse(
        fs.readFileSync(
          `${currentDirectory}/packfiles/${pack_url}/metadata.json`,
          "utf8",
        ),
      );
    } else {
      pack_data = [];
      console.warn(
        "[getPackMetaData] No metadata.json found for pack: " + pack_url,
      );
    }
    resolve(pack_data);
  });
...
function checkForValidDBDPath() {
  if (!fileExists(getDBDPathSync() + "/DeadByDaylight.exe")) {
    ...
    if (path == "") {
      dialog.showErrorBox(
        "No game path",
        `Please set your Dead by Daylight installation path in the options.`,
      );
      return false;
    }
    ...
    dialog.showErrorBox(
      "Invalid game path",
      `The path (${path}) set is invalid. Please verify your Dead by Daylight installation path.`,
    );
    return false;
  }
  return true;
