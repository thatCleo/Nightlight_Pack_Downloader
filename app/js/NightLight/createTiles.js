...
function setPackTiles(json) {
  ...
  if (packview.length == 0) {
    document.getElementById("info").innerText =
      `Error: packview.length = ${packview.length}`;
    return;
  }
  ...
  variants_data.forEach((variant) => {
    ...
    packs_with_variants[pack_index][1].push([
      variant.url,
      variant.variant_nickname,
    ]);
  });
  ...
  packData.forEach((pack) => {
    ...
    const default_variant_index = Number(
      (variants.length / 2).toString().split(".")[0],
    );
    ...
    if (variants.length >= 2) {
      variants_template = `<div class="button_variant button_variant_prev">
      ...
      variants[default_variant_index].childNodes[0].classList.add(
      ...
      variants[default_variant_index + 1].childNodes[0].classList.add(
        "description_variant_visible",
      );
    }
    ...
    for (let i = 0; i < pack.has.length; i++) {
      if (packContent == "") packContent += `${formatText(pack.has[i])}`;
      else packContent += `, ${formatText(pack.has[i])}`;
    }
    const tile = `
<div id="pack-banner-${pack.id}" class="_1he3xh0 pack_tile"><img src="${window.directory.currentPath()}/cached_images/${pack.id}_${pack.current_version}/banner.png"
  ...
  <div class="_1he3xh4">
    ...
    variant_container.parentNode.insertBefore(
      avatar_elements,
      variant_container.nextElementSibling,
    );

    if (variants_template != "") {
      const variant_container = packTile.getElementsByClassName(
        `variants-${pack.id}`,
      )[0];
...
async function downloadAvatar(user_id, avatar_id, pack_id) {
  ...
  fs.access(`${directoryPath}/${fileName}`, fs.constants.F_OK, async (err) => {
    if (err) {
      ...
      const avatar_img = document.getElementById(
        `pack-avatar-${pack_id}-${user_id}`,
      );
      if (avatar_img) {
