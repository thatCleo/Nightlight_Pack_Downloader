...
async function setPackOrderTiles_Manage(url) {
    ...
    await window.packFunctions.getPackMetaData(url)
        .then(data => {
            ...
            let pack_content_elements = [];
            for (let i = 0; i < pack_data.has.length; i++) {
                    let pack_content_element = document.createElement('p');
                    pack_content_element.classList.add('pack-order-info-content-label');
                    pack_content_element.classList.add(`url-${url}`);
                    pack_content_element.classList.add(`content-${formatText(pack_data.has[i])}`);
                    pack_content_element.innerText = formatText(pack_data.has[i]);

                    pack_content_elements.push(pack_content_element);
            }
            ...
            const tile = `
                <div class="pack-order">
                    ...
                    <div class="pack-order-info">
                        ...
                        <div class="pack-order-info-content-container"></div>
                        </div>
                    </div>
            ...
            const pack_content_display = packTile.getElementsByClassName('pack-order-info-content-container')[0];
            pack_content_elements.forEach(element => {
                pack_content_display.append(element);
